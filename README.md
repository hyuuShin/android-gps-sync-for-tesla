...
**처음 사용하는 경우 [화면별 시작 가이드](docs/FIRST_TIME_SETUP.md)를 먼저 읽으세요.** Cloudflare 호스팅 계정·공개 주소 준비 → Tesla Developer 앱 등록·결제 설정 → 인증 서버 배포 순서로, 화면 메뉴와 입력값을 안내합니다. 각자 본인 계정으로 준비해야 합니다.
